...
**Guide**
Reference audio used for timing/pitch reference while recording, from one of two sources: **imported** by the user (e.g., a backing track), or a **Metronome** generated by the tool. Both play the same structural role — audible in the Monitor Mix, anchors the Project timeline's t=0, defines the beat grid — differing only in provenance. Has no Takes and carries no performance — it is not a Track. Excluded from a Mixdown by default.

**Metronome**
A Guide generated by the tool rather than imported — a click, not a reference recording the musician brought in. Distinct in the musician's mind from an imported Guide even though it fills the same role: it's a tangible feature the tool provides, not a file they sourced themselves. Deliberately simple for now: a single constant tempo and time signature for the whole Metronome, no mid-Guide tempo or time-signature changes — that complexity is not yet supported.
_Avoid_: Click track (implies the tempo/meter-change flexibility a real click-track feature would have, which this doesn't support yet)
